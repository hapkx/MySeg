...
class GatedFusionModule(nn.Module):
    """
    更稳的门控融合：
    1) 先把 PVT 与 U-Net 特征投影到同一通道数；
    2) 用轻量门控生成逐通道注入权重；
    3) 通过 zero-init 残差 + 可学习标量 alpha 控制注入强度；
    4) 初始状态严格接近恒等映射，避免一开始破坏扩散主干。
    """
...
class UNetModel(nn.Module):
    def __init__(
        ...
        mask_channels=None,
        pvt_fusion_levels=(8,),
        trainable_pvt_stages=(),
        cross_attn_context_stage=1,
        feb_levels=(8,),
    ):
        super().__init__()
        ...
        self.trainable_pvt_stages = tuple(trainable_pvt_stages)
        self.cross_attn_context_stage = cross_attn_context_stage
        ...
        if self.use_pvt_fusion or self.use_cross_attn:
            ...
            if (not freeze_pvt) and len(self.trainable_pvt_stages) > 0:
                train_prefixes = []
                for stage_id in self.trainable_pvt_stages:
                    if stage_id == 0:
                        train_prefixes.append("pvt.patch_embed")
                    train_prefixes.append(f"pvt.stages_{stage_id}")
                for name, param in self.pvt_extractor.named_parameters():
                    if any(name.startswith(prefix) for prefix in train_prefixes):
                        param.requires_grad = True

            self.pvt_trainable = any(p.requires_grad for p in self.pvt_extractor.parameters())
            ...
            if self.use_cross_attn:
                self.middle_cross_attn = CrossAttentionBlock(
                    ...
                    context_channels=pvt_channels[self.cross_attn_context_stage],
                    num_heads=num_heads,
                    use_checkpoint=use_checkpoint,
                )
        else:
    ...
    def forward(self, x, timesteps, y=None):
        ...
        for block_id, module in enumerate(self.input_blocks):
            h = module(h, emb)
            if pvt_feats is not None and str(block_id) in (self.fusion_modules or {}):
                spec = self.fusion_block_specs[block_id]
                h = self.fusion_modules[str(block_id)](pvt_feats[spec["pvt_idx"]], h)
            hs.append(h)

        h = self.middle_block(h, emb)
        if self.middle_cross_attn is not None and pvt_feats is not None:
            h = self.middle_cross_attn(h, pvt_feats[self.cross_attn_context_stage])

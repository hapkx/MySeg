"""
UNet with Cross-Branch Attention Fusion Module (v2)

使用交叉分支注意力替换原有的门控融合

特点：
- 多头自注意力机制
- UNet作Query，PVT作Key/Value
- 更强的表现力

使用方式：
  from guided_diffusion_new.unet_gate_fusion_v2 import UNetModelWithFusionV2

  model = UNetModelWithFusionV2(
      image_size=256,
      in_channels=4,
      model_channels=128,
      out_channels=3,
...
from .fusion_modules import CrossBranchAttentionModule
import torch
import torch.nn as nn


class UNetModelWithFusionV2(UNetModel):
    """
    使用Cross-Branch Attention (CBA)融合模块的UNet

    对原有UNetModel的改进：
    1. 将GatedFusionModule替换为CrossBranchAttentionModule
    2. 使用多头自注意力建模PVT-UNet交互
    3. 更复杂但表现力更强的融合策略
    """
    ...
    def _replace_fusion_modules(self):
        """
        将GatedFusionModule替换为CrossBranchAttentionModule
        ...
        for i, fusion_module in enumerate(self.fusion_modules):
            ...
            # 创建新的CrossBranchAttentionModule
            # CBA模块需要统一的维度（dim）
            new_fusion = CrossBranchAttentionModule(
                dim=unet_channels,
    ...
    def print_fusion_info(self):
        """打印融合模块信息"""
        print("=" * 60)
        print("Fusion Module Information (Cross-Branch Attention v2)")
        print("=" * 60)

        if hasattr(self, 'fusion_modules'):
            for i, fusion_module in enumerate(self.fusion_modules):
                print(f"Fusion {i}: {fusion_module.__class__.__name__}")
                if hasattr(fusion_module, 'num_heads'):
                    print(f"  Attention heads: {fusion_module.num_heads}")
                    print(f"  Dimension:       {fusion_module.dim}")
        else:

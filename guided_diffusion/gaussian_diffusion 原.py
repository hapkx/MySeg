...
class GaussianDiffusion:
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        """
        # print("DEBUG self.loss_type =", self.loss_type)
        # print("DEBUG type(self.loss_type) =", type(self.loss_type))
        # print("DEBUG LossType =", LossType)
        # print("DEBUG self.loss_type == LossType.MSE ->", self.loss_type == LossType.MSE)
        # print("DEBUG self.loss_type.__class__ is LossType ->", self.loss_type.__class__ is LossType)
        # print("DEBUG self.loss_type.name =", getattr(self.loss_type, "name", None))
        ...
        pred_probs = th.softmax(pred_xstart, dim=1)         # [B, 3, H, W]

        class_weights = th.tensor([0.2, 1.0, 1.0], device=pred_xstart.device)

        # 逐样本 CE
        seg_ce = F.cross_entropy(pred_xstart, gt_classes, weight=class_weights, reduction="none").mean(dim=(1, 2))                    # [B]

        # 逐样本 Dice
        seg_dice = multiclass_dice_loss(pred_probs, mask)   # [B]
        ...
        # 低噪声时间步更强调分割监督，高噪声时间步减弱
        seg_time_weight = 1.0 - t.float() / self.num_timesteps   # [B]
        seg_loss = seg_time_weight * (seg_ce + seg_dice)         # [B]

...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            for block_idx in range(num_res_blocks):
                ...
                # 如果以后要加 FEB，只在深层 ds=4,8 后面加，而不是替换整个 encoder
        ...
        if self.use_cross_attn:
            ...
            self.middle_cross_attn = CrossAttentionBlock(
                channels=ch,
                context_channels=256,   # pvt_v2_b0 F4
                num_heads=num_heads,
                use_checkpoint=use_checkpoint,
            )
        ...
        if self.use_pvt_fusion or self.use_cross_attn:
            self.pvt_extractor = PVTv2FeatureExtractor(
                pretrained=True,
                local_ckpt_path='/home/nas2/biod/piankexin/AAAablation_model/guided_diffusion/pvt_v2_b0.pth',
            )
            ...
            # 你的 channel_mult 如果是 1,2,5,8 且 num_channels=32
            # 对应 stage channels = [32,64,160,256]
            self.fusion_stage_channels = [model_channels * mult for mult in channel_mult]

            # 只做两层融合:
            # F1 -> ds4 -> stage index 2
            # F2 -> ds8 -> stage index 3
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.pvt_extractor is not None:
            # 输入默认为 [B, 4, H, W] = [image(1), mask(3)]
            img_ch = self.in_channels - 3
            x_img = x[:, :img_ch, :, :]   # 取原图部分
        ...
        if self.use_cross_attn:
            ...
            h = self.middle_cross_attn(h, pvt_feats[3])   # F4 only in middle

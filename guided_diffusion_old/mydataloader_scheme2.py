...
核心思路：
  ...
  - PVT输入：256×256 patch（相同位置的patch）
  - 优点：完全对齐，实现最简单，坐标问题完全解决
  - 缺点：PVT失去全局上下文，仅看局部信息

训练模式：
  1) full-image分支：256×256整图，PVT/UNet都输入这个整图
  2) patch分支：256×256 patch，PVT/UNet都输入这个patch
"""
...
# =========================================================
# Dataset - 方案2：PVT也输入相同patch
# =========================================================
class MyDatasetScheme2(Dataset):
    """
    【方案2】相同patch版本

    返回值:
      img_patch: (1, 256, 256) - UNet的patch输入，同时也是PVT的输入
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'training':
            ...
            if use_full_image:
                ...
                img_patch_tensor = img_patch_tensor * 2.0 - 1.0

                mask_onehot = self._mask_to_onehot(mask_patch)

                # 【方案2关键】PVT和UNet都使用同一个patch，坐标完全对齐
                return img_patch_tensor, mask_onehot, name
